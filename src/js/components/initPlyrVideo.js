        ...
        <div class="plyr__controls">
            <button type="button" class="plyr__control js-hovered" aria-label="Play, {title}" data-plyr="play">
                ...
                <svg class="icon--pressed"  width="13" height="17" viewBox="0 0 13 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <line x1="1.5" y1="1.5" x2="1.5" y2="14.7045" stroke="white" stroke-width="2" stroke-linecap="round"/>
                    <line x1="10.7597" y1="1.5" x2="10.7597" y2="14.7045" stroke="white" stroke-width="2" stroke-linecap="round"/>
                </svg>
            </button>
            <div class="plyr__progress">
                <div class="plyr__progress__input-wrap">
                    ...
                    <svg width="30" height="6" viewBox="0 0 30 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <line x1="6.10352e-05" y1="3.26904" x2="30.0001" y2="3.26904" stroke="white" stroke-width="3"/>
                    </svg>
                    </div>
                </div>
        ...
        elements.forEach(p => {
            ...
            player.on('ready', event => {
                ...
                if (playerWrapper?.classList.contains('has-fullscreen')) {
                    ...
                    //     }
                    // }
                    // addSources();const pausePlaying = () => elements.forEach(p => {if (!p.paused) p.pause()});

                    openFullscreen.addEventListener('click', (e) => {
                        scrollHide();
                        modal.classList.add('is-visible');elements.forEach(p => {
                            console.log(p)
                            const pWrapper = p.plyr.elements.wrapper;
                            const pModal = pWrapper.closest('.js-modal');    if (pModal && pModal.classList.contains('is-visible')) {
                                p.plyr.play();
                    ...
                    const closeAll = (modal) => {
                        if (modal.classList.contains('is-visible')) {
                            const thisPlayer = modal.querySelector('.plyr');    scrollShow();
                            modal.classList.remove('is-visible');    if (thisPlayer) {
